...
const HomeScreen = () => {
    const [isActiveCategory,setActiveCategory] = useState('Beef')
        ...
        try {
         const response = await axios.get('https://www.themealdb.com/api/json/v1/1/categories.php')
            ...
            if(response && response.data){
                // console.log("response----value",response.data.categories)
    ...
    const getRecipes = async(category='Beef')=>{
        try {
         const response = await axios.get(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${category}`)
            ...
            console.log("recirpeeeeee-result====",response.data)
    ...
    <View className="flex-1 bg-white">
        ...
        >
            <View className=" mx-4 flex-row justify-between items-center mb-2">
                <Image source={require("../images/avatar.jpg")} style={{width: hp(5.5) ,height:hp(5)}} />
                <BellIcon size={hp(4)} color="gray"></BellIcon>
            </View>

            <View className="mx-4 space-y-2 mb-2">
                <Text style={{fontSize:hp(1.7)}} className=" text-neutral-600">Hello Ajay singh !</Text>
                <View>
